...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    async function loadUser(): Promise<void> {
      try {
        const res: Response = await fetchWithAuth(
          `${NEXT_PUBLIC_AUTH_SERVICE_URL}/api/user`,
        );
        if (!res.ok)
          throw new Error(`Response status ${res.status} not expected`);
    ...
    async function loadTenant(): Promise<void> {
      try {
        const res: Response = await fetchWithAuth(
          `${NEXT_PUBLIC_AUTH_SERVICE_URL}/api/tenant`,
        );
        if (!res.ok)
          throw new Error(`Response status ${res.status} not expected`);
    ...
    async function loadTenants(): Promise<void> {
      try {
        const res: Response = await fetchWithAuth(
          `${NEXT_PUBLIC_AUTH_SERVICE_URL}/api/tenant/all`,
        );
        if (!res.ok)
          throw new Error(`Response status ${res.status} not expected`);
    ...
    async function loadData() {
      try {
        const authRes = await fetchWithAuth(
          `${NEXT_PUBLIC_AUTH_SERVICE_URL}/api/auth/status`,
        );
        ...
        if (authRes.status === 403) {
          ...
          if (window.location.pathname === "/") return;
          if (window.location.pathname === "/error") return;
          if (window.location.pathname !== "/connect-shopify")
        ...
        if (!authRes.ok)
          throw new Error(
            `Error fetching auth status. Response status ${authRes.status} not expected`,
          );
        if (window.location.pathname === "/login")
  ...
  const logout = async () => {
    ...
    await fetchWithAuth(`${NEXT_PUBLIC_AUTH_SERVICE_URL}/api/auth/logout`, {
      method: "DELETE",
    });
    log.info("Logged out");
  ...
  const switchTenant = async (tenantId: string | null) => {
    const tenantToSwitchTo: Tenant | undefined = tenants.find(
      (t) => t.id === tenantId,
    );
    if (!tenant) {
    ...
    const res = await fetchWithAuth(
      `${NEXT_PUBLIC_AUTH_SERVICE_URL}/api/auth/shopify/switch`,
      {
        method: "POST",
